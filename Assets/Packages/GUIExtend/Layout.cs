...
namespace Packages.GUIExtend
{
	...
	/// 样式定义
	/// </summary>
	public class Layout : ClassExtension
	{
		/// <summary>
		/// 隐式定义值类型(便于int/float/bool类型的转换)
		/// </summary>
		public struct Val 
		{
			public float val;
		...
		/// 当前区域<BR/>
		/// 备注：当前样式的最大区域<BR/>
		/// background rect of layout. Height increases every new line (or stays fixed, which is max)
		/// </summary>
		public Rect field; 
		/// <summary>
		/// 游标<BR/>
		/// 备注：宽度为0，且坐标是相对于field的坐标<BR/>
		/// rect with zero width. X and Y are relative to field<BR/>
		/// 可用于当前要绘制的位置。
		/// </summary>
		public Rect cursor;
		/// <summary>
		...
		/// <summary>
		/// 取得Inspector视图中下一个绘制范围
		/// </summary>
		/// <returns>Inspector视图中下一个绘制范围</returns>
		...
		public static void SetInspectorRect (Rect iRect)
		{
			if (Event.current.type == EventType.Layout) GUILayoutUtility.GetRect(1, iRect.height, "TextField");
		}

		/// <summary>
		/// 标杆
		/// </summary>
		/// <param name="iPadding">边距</param>
		/// <param name="iHeight">高度</param>
		public void Par(
			RectOffset iPadding,
			Val iHeight = new Val())
		{
			Par(iHeight, iPadding.left, (iPadding.top + iPadding.bottom)/ 2.0f);
		}
		...
		/// </summary>
		/// <param name="iHeight">高度</param>
		/// <param name="iMargin">空白：左</param>
		/// <param name="iPadding">边距：上&下</param>
		public void Par (
			Val iHeight = new Val(),
			Val iMargin = new Val(),
			Val iPadding = new Val() )
		{ 
			var heightTmp = iHeight.ovd? (int)iHeight.val : lineHeight;
			var marginTmp = iMargin.ovd? (int)iMargin.val : margin;
			var paddingTmp = iPadding.ovd? (int)iPadding.val : verticalPadding;

			// 设定矩形区域
			// 游标
			cursor = new Rect(
				field.x + marginTmp, cursor.y + cursor.height + paddingTmp, 
				0, heightTmp - paddingTmp);
			
			// 区域
			// field = new Rect(field.x, field.y, field.width, Mathf.Max(field.height, cursor.y + cursor.height));
			field.height = Mathf.Max(field.height, cursor.y + cursor.height);
		}

		/// <summary>
		/// 内边距设定
		/// </summary>
		/// <param name="iWidth">宽度(百分比:0.0f~1.0f)</param>
		/// <param name="iMargin">空白：左</param>
		/// <param name="iRightMargin">空白：右</param>
		/// <param name="iPadding">边距：左&右</param>
		/// <returns></returns>
		public Rect Inset (
			Val iWidth = new Val(), 
			Val iMargin = new Val(), 
			Val iRightMargin = new Val(), 
			Val iPadding = new Val() )
		{
			var marginTmp = iMargin.ovd? (int)iMargin.val : margin;
			var rightMarginTmp = iRightMargin.ovd? (int)iRightMargin.val : rightMargin;
			var paddingTmp = iPadding.ovd? (int)iPadding.val : verticalPadding;
			
			var widthTmp = iWidth.ovd? iWidth.val : 1;
			widthTmp = 1.00f >= widthTmp ? 1.0f : widthTmp;
			// 实际的宽度为当前最大区域宽度(除去左右空白) * 百分比
			widthTmp *= field.width - marginTmp - rightMarginTmp; 
			
			// 设定区域
			lastRect = new Rect (cursor.x, cursor.y + field.y, widthTmp - paddingTmp, cursor.height);
			cursor.x += widthTmp;
			return lastRect;
		}
		...
		/// <param name="iRightMargin"></param>
		/// <param name="iVerticalPadding"></param>
		/// <param name="iHorizontalPadding"></param>
		/// <returns></returns>
		public Rect ParInset (
			...
			Val iMargin = new Val(), 
			Val iRightMargin = new Val(), 
			Val iVerticalPadding = new Val(),
			Val iHorizontalPadding = new Val() )
		{ 
			Par(iHeight, iMargin, iVerticalPadding); 
			return Inset(iWidth, iMargin, iRightMargin, iHorizontalPadding); 
		}
		...
		#region Icon
			...
			public bool Icon (Texture2D texture, Rect rect, IconAligment horizontalAlign=IconAligment.resize, IconAligment verticalAlign=IconAligment.resize, bool frame=false, bool tile=false, bool clickable=false, bool alphaBlend=true)
			{
				...
				if (!tile) 
				{
					if (alphaBlend) UnityEngine.GUI.DrawTexture(ToDisplay(rect), texture, ScaleMode.ScaleAndCrop); 
					#if UNITY_EDITOR
				...
				else
				{
					...
					for (float x=0; x<rect.width; x+=texture.width*zoom)
						...
						{
							if (alphaBlend) UnityEngine.GUI.DrawTexture(new Rect(x+localRect.x, y+localRect.y, texture.width*zoom, texture.height*zoom), texture, ScaleMode.StretchToFill);
							#if UNITY_EDITOR
		...
		#region Fields
			...
			//	GUI.SetNextControlName("LayoutField");
				...
				if (type == typeof(float)) 
				{
					...
					{
						float newval = val;
						if (!quadratic) newval = UnityEngine.GUI.HorizontalSlider(ToDisplay(sliderRect), val, min, max); 
						else newval = Mathf.Pow(UnityEngine.GUI.HorizontalSlider(ToDisplay(sliderRect), Mathf.Pow(val,0.5f), Mathf.Pow(min,0.5f), Mathf.Pow(max,0.5f)),2); 
				...
				else if (type == typeof(int)) 
				{
					...
					if (slider)
					{
						if (!quadratic) val = (int)UnityEngine.GUI.HorizontalSlider(ToDisplay(sliderRect), val, (int)min, (int)max); 
						else val = (int)Mathf.Pow(UnityEngine.GUI.HorizontalSlider(ToDisplay(sliderRect), Mathf.Pow(val,0.5f), Mathf.Pow(min,0.5f), Mathf.Pow(max,0.5f)),2); 
					}
				...
				string tooltip=null )
			{
				...
				{
					if (UnityEngine.GUI.Button(ToDisplay(rect), content, urlStyle)) Application.OpenURL(url); 
				...
				string tooltip=null )
			{
				...
				bool result = false;
				if (style==null) result = UnityEngine.GUI.Button(ToDisplay(rect), content, buttonStyle);
				else result = UnityEngine.GUI.Button(ToDisplay(rect), content, style);
				...
				string tooltip=null )
			{
				...
				bool dst = UnityEngine.GUI.Toggle(ToDisplay(rect), src, content, buttonStyle);
		...
		#region Layered
			...
			public void DrawArrayAdd<T> (ref T[] layers, ref int selected, Rect rect, bool reverse=false, Func<T> createElement=null, Action<int> onAdded=null, GUIStyle style=null)  //createElement is called to return element before it was added. onAdded called after the element has been added.
			{
				...
				{ 
					...
					if (selected >= 0)
					{
						layers = ArrayTools.Insert(layers, selected + (reverse?1 : 0), iCreateElement:createElement);
						if (reverse) selected++;
					...
					{
						layers = ArrayTools.Add(layers, iCreateElement:createElement);
			...
			public void DrawArrayDown<T> (ref T[] layers, ref int selected, Rect rect, bool dispUp=false, System.Action<int,int> onSwitch=null, GUIStyle style=null) //where T : ILayer
			{
				...
				{ 
					if (OnBeforeChange != null) OnBeforeChange();
					ArrayTools.Swap(layers, selected, selected+1);
					selected++; 
			...
			public void DrawLayer (Action<Layout,bool,int> onGUI, ref int selectedNum, int num, int heightSpacing=3, int widthOffset=3)  //heightSpacing - vertical margins from contents expanding background. WidthOffset - horizontal offset from field borders shrinking background
			{
				...
				Rect backgroundRect = GetBackgroundRect( (Layout tmp) => { Par(heightSpacing,iMargin:0); onGUI(this, curIsSelected, num); Par(heightSpacing,iMargin:0); } );
			...
			public void DrawLayer (Action<Layout,bool,int,object> onGUI, ref int selectedNum, int num, object parent, int heightSpacing=3, int widthOffset=3)  //heightSpacing - vertical margins from contents expanding background. WidthOffset - horizontal offset from field borders shrinking background
			{
				...
				//calculating background rect
				Rect backgroundRect = GetBackgroundRect( (Layout tmp) => { Par(heightSpacing,iMargin:0); onGUI(this, curIsSelected, num, parent); Par(heightSpacing,iMargin:0); } );
